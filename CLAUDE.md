...
Noa Studio is a browser-based DAW under active construction. As of Phase 6b, audio is produced by **WASM plugins** loaded through the Noa Plugin ABI v1 (with v1.1 preset-hot-swap extensions, see `docs/plugin-abi-v1.md`). Two built-in plugins ship in `src/builtin-plugins/`: `com.noa.sine` (8-voice polyphonic sine generator with a 3-preset bank demonstrating glitch-free hot-swap) and `com.noa.gain` (linear gain insert). The audio worklet runs a `MixerRouter` over multiple `PluginChain`s — one per track-with-generator plus one per channel-with-FX — and fans each channel's post-fader signal out into every entry of its `sends` list (master + drum bus + reverb bus, etc). Transport state lives in the worklet: it owns `playheadSamples` / `bpm` / `loopStart..End`, advances per block, wraps sample-accurately at the loop boundary, and publishes a 4×u32 telemetry SAB the main thread reads each RAF. A main-thread `ClipScheduler` looks ahead ~50 ms and pushes NoteOn/NoteOff events with absolute sample-time onto the engine ring. A per-instance JS `Worker` handles slow non-RT work like preset compilation. Each plugin can ship a floating HTML UI hosted in a sandboxed iframe. A Service Worker at `/plugin-cache-sw.js` serves plugin assets out of OPFS under `/_noa/*`; a Browser-pane install flow lets users add new plugins from a `.noaplugin` ZIP URL. Project state lives in a `SharedWorker` in `src/coordinator/`, persisted to OPFS. See `docs/superpowers/plans/2026-05-17-noa-daw-roadmap.md` for what's deferred (track-level FX inserts, audio clip PCM playback, generators on tracks t2-t8, UI dispatches for clip drag, audio clip PCM playback, send-level UI).
