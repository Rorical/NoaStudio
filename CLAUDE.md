...
- `npm test` — Vitest unit tests (Node environment, no browser). All 254 tests across 22 suites (engine + coordinator + sw + built-in plugins) should pass.
...
Noa Studio is a browser-based DAW under active construction. As of Phase 6a, audio is produced by **WASM plugins** loaded through the Noa Plugin ABI v1 (with v1.1 preset-hot-swap extensions, see `docs/plugin-abi-v1.md`). Two built-in plugins ship in `src/builtin-plugins/`: `com.noa.sine` (8-voice polyphonic sine generator with a 3-preset bank demonstrating glitch-free hot-swap) and `com.noa.gain` (linear gain insert). The audio worklet runs a `MixerRouter` over multiple `PluginChain`s — one per track-with-generator plus one per channel-with-FX — so every track outputs through its mixer channel's FX rack and onward through the bus graph (sends[0]) to master. A main-thread `ClipScheduler` looks ahead ~50 ms and pushes NoteOn/NoteOff events with absolute sample-time onto the engine ring; the worklet dispatches them sample-accurately within the right render quantum. A per-instance JS `Worker` handles slow non-RT work like preset compilation. Each plugin can ship a floating HTML UI hosted in a sandboxed iframe. A Service Worker at `/plugin-cache-sw.js` serves plugin assets out of OPFS under `/_noa/*`; a Browser-pane install flow lets users add new plugins from a `.noaplugin` ZIP URL. Project state lives in a `SharedWorker` in `src/coordinator/`, persisted to OPFS. Per-channel mixer meters now reflect real engine audio. See `docs/superpowers/plans/2026-05-17-noa-daw-roadmap.md` for what's deferred (track-level FX inserts, multi-destination sends, audio clip PCM playback, engine-driven transport, UI-dispatches-to-coordinator).
...
- `MixerRouter.ts` (Phase 6a) — Multi-chain routing graph hosted by the worklet. Owns `Map<chainId, PluginChain>` plus a `RoutingConfig` (tracks → channels → bus sends → master). `processBlock` runs the per-track generator chains into a channel input mix, walks channels in topological order (master last), applies vol/pan, accumulates into `sendTo` destinations, and emits a per-channel peak/RMS meter. Linear pan to avoid cascading centre attenuation. v1 honours only `sends[0]`.
