...
- `npm test` — Vitest unit tests (Node environment, no browser). All 340 tests across 26 suites (engine + coordinator + sw + built-in plugins) should pass.
...
A main-thread `ClipScheduler` looks ahead ~50 ms and pushes NoteOn/NoteOff events with absolute sample-time onto the engine ring; on stop it emits panic NoteOffs so ringing voices release immediately. A `MidiInput` decodes Web MIDI 1.0 status bytes into the same NoteOn/Off events (enable via `window.__noa.enableMidi(trackId)`).
...
- `ClipScheduler.ts` (Phase 6a, refined 6c+) — Main-thread look-ahead MIDI sequencer. Each tick reads the engine sample counter, derives the current beat from a fixed `(startSample, startBeat)` anchor, scans clips up to a small horizon, and pushes paired NoteOn/NoteOff frames with absolute sample-time onto the engine event ring. Tracks `active` notes (those whose paired NoteOff is still in the future); `stop()` emits an immediate NoteOff for each so voices don't ring forever when transport halts mid-clip. Loop wraps are handled by the caller invoking `reset({startSample, startBeat})`.
