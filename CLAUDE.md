...
- `npm test` — Vitest unit tests (Node environment, no browser). All 476 tests across 33 suites (engine + coordinator + sw + built-in plugins) should pass.
...
Noa Studio is a browser-based DAW under active construction. Audio is produced by **WASM plugins** loaded through the Noa Plugin ABI v1 (with v1.1 preset-hot-swap extensions, see `docs/plugin-abi-v1.md`). Two built-in plugins ship in `src/builtin-plugins/`: `com.noa.sine` (8-voice polyphonic sine generator with a 3-preset bank demonstrating glitch-free hot-swap) and `com.noa.gain` (linear gain insert). The seed assigns sine to every MIDI track so hitting Play produces the full demo song.

The audio worklet runs a `MixerRouter` over many `PluginChain`s — one per track (slot 0 = generator, 1..N = pre-channel FX inserts) and one per channel that has FX. Each channel's post-fader signal fans out into every entry of its `sends[]` list, with optional per-destination levels from `sendLevels`. Per-FX bypass is honored by the chain. Transport state lives in the worklet: it owns `playheadSamples` / `bpm` / `loopStart..End`, advances per block, wraps sample-accurately at the loop boundary, and publishes a 4×u32 telemetry SAB the main thread reads each RAF.

A main-thread `ClipScheduler` looks ahead ~50 ms and pushes NoteOn/NoteOff events with absolute sample-time onto the engine ring; on stop it emits panic NoteOffs so ringing voices release immediately. A sibling `AudioClipScheduler` does the same for **audio clips** — clips with a `sampleId` emit `EVT_AUDIO_ON/OFF` frames that a worklet-side `AudioClipPlayer` turns into real PCM playback (decoded samples mixed into the clip's channel bus). Both schedulers anchor on the engine's **monotonic** sample clock (`currentMonotonicSamples()`), not the loop-wrapping playhead, so events dispatch correctly across loop wraps. A `MidiInput` decodes Web MIDI 1.0 status bytes into the same NoteOn/Off events (enable via `window.__noa.enableMidi(trackId)`).

The engine's plugin lifecycle is driven by a **diff-sync effect** in App.jsx: every coordinator change to track generators, track effects, or channel effects reconciles against what's loaded in the engine — drag-to-track and × on an FX card just work. Coordinator-side param changes also forward into the engine through a snapshot-diff effect. Plugins not in the Vite-bundled registry (user-installed `.noaplugin` files) are fetched on demand via `PluginRegistry.loadFromOpfsViaSw`.

A per-instance JS `Worker` handles slow non-RT work like preset compilation. Each plugin can ship a floating HTML UI hosted in a sandboxed iframe. A Service Worker at `/plugin-cache-sw.js` serves plugin assets out of OPFS under `/_noa/*`; a Browser-pane install flow lets users add new plugins from a `.noaplugin` ZIP URL.

Project state lives in a `SharedWorker` in `src/coordinator/`, persisted to OPFS. A `schemaVersion` field gates incompatible saved projects on load — they're discarded and reseeded rather than half-loaded. Audio-clip PCM playback + real waveforms shipped in Phase 7 (`docs/superpowers/plans/2026-06-11-phase-7-audio-clip-playback.md`); still deferred: multi-version per plugin id.

Keyboard shortcuts: **Space** = play/stop, **Esc** = stop + rewind, **Home** = rewind to 0, **L** = toggle loop, **M** / **S** = toggle mute/solo on the selected track, **F2** = rename the selected clip (or the selected track when no clip is selected), **Delete** / **Backspace** = delete the selected clip, **Cmd/Ctrl-D** = duplicate the selected clip, **Cmd/Ctrl-Z** = undo, **Cmd/Ctrl-Shift-Z** / **Cmd/Ctrl-Y** = redo, **←** / **→** = select previous / next clip on the same track, **↑** / **↓** = jump to the nearest clip on the track above / below. Click a track header in the Playlist to select it. **Alt-click** a clip to duplicate it in place. Double-click a clip header or a track name to start renaming. Shortcuts bail when an editable element (INPUT, TEXTAREA, contenteditable) is focused.
...
2. **Meter loop** (always running): reads real engine data for **every** channel. `engine.readMeters()` drains the meter ring (one 16-byte frame per channel per block, keyed by `channelHash`); a `hashToChannelId` map inverts the hash back to the channel id; the loop folds the max `peak` per channel per RAF tick and writes `levels[id]` / `levels[id + '_r']` with snap-up + ~15%/RAF decay (`Math.max(prior * 0.85, fresh)`). All channels (Master included) are real — the old `sin()`/`Math.random()` simulation was removed once `MixerRouter` emitted per-channel meters. The meter is currently mono: `MixerRouter` emits a single scalar peak per channel, so `levels[id]` and `levels[id + '_r']` carry the same value (a true stereo split would mean emitting peakL/peakR — the unused `rms` bytes in the frame are available for it).
...
Clips are either audio (`clip.audio === true`, with a `clip.sampleId` pointing into `project.samples`; `ClipWaveform` renders a real min/max envelope from precomputed peaks) or MIDI (`clip.pattern.notes` rendered as `ClipMidiPreview`). MIDI notes are stored as compact tuples `[beat, pitch, length]` in `data.js` and inside `clip.pattern.notes`. The `PianoRoll` component inflates them to `{id, beat, pitch, length, velocity}` for editing and serializes back to tuples via `onUpdateNotes` on every change.

Audio clips get their PCM two ways: the demo seeds one procedurally-synthesized sample (`generateDemoSample`, regenerated each boot), and users **drop an audio file onto a track lane** → it's decoded to engine-rate interleaved PCM (`decodeAudioFile`, resampled via `OfflineAudioContext`), persisted to an OPFS `SampleStore`, and added as a `sampleId`-bearing audio clip via the `IMPORT_AUDIO` action. App.jsx's sample-materialize effect resolves each `project.samples` entry to PCM (synth → regenerate, import → reload from OPFS), computes waveform peaks (`computePeaks`), and ships the buffer to the worklet via `engine.loadSample` (a transfer — peaks are computed first). Deleting an audio clip GCs its sample when no other clip references it; a prune effect frees the worklet PCM, peaks, and OPFS blob.
...
- `audio-worklet.ts` — `AudioWorkletProcessor` shim. Drives a `MixerRouter` (Phase 6a), drains the engine event ring (Transport / Tempo events go to the worklet's internal `TransportState`, NoteOn/Off/ParamSet route to chains via `MixerRouter.queueEvent`), advances the playhead per block when playing, wraps sample-accurately at the loop boundary (Phase 6b), publishes per-block meter frames + a 4×u32 telemetry SAB (`playheadSamples` / `playheadBeats` / `flags` / `blockCounter`).
- `EngineClient.ts` — Main-thread façade. Owns the `AudioContext` + `AudioWorkletNode`. Requires `crossOriginIsolated === true` (enforced by COOP/COEP headers in `vite.config.js`). Exposes `loadPlugin` / `unloadInstance` (via `WorkletProtocol`), `setParam` / `noteOn`, `updateRouting` / `setLoop`, `loadSample` / `unloadSample` (ship/drop audio-clip PCM, transferred), `playheadBeats()` / `isPlaying()`, and `currentMonotonicSamples()` (the never-resetting block-counter×128 clock the schedulers anchor on) reading the 4×u32 telemetry SAB.
...
- `PluginChain.ts` — Linear signal chain hosted by the worklet. `install(slot, instance)` / `uninstall(slot)` / `queueEventFrame(slot, frame)` / `processBlock(blockSize, outBus, inBus?)`. Slot 0 conventionally holds the generator; slots 1..N are insert FX taking the previous slot's output as input. For FX-only chains (channel FX racks) the optional `inBus` feeds the first FX. Many chains live in parallel inside `MixerRouter` (Phase 6a).
- `MixerRouter.ts` (Phase 6a, refined in 6b) — Multi-chain routing graph hosted by the worklet. Owns `Map<chainId, PluginChain>` plus a `RoutingConfig` (tracks → channels → bus sends → master). `processBlock` runs the per-track generator chains into a channel input mix, walks channels in topological order (master last), applies vol/pan, fans each channel's output into every destination in `sendsTo` (optionally scaled by `sendsLevels`), and emits a per-channel peak/RMS meter. Linear pan to avoid cascading centre attenuation.
- `ClipScheduler.ts` (Phase 6a, refined 6c+) — Main-thread look-ahead MIDI sequencer. Each tick reads the engine's monotonic sample counter, derives the current beat from a fixed `(startSample, startBeat)` anchor, scans clips up to a small horizon, and pushes paired NoteOn/NoteOff frames with absolute sample-time onto the engine event ring. Tracks `active` notes (those whose paired NoteOff is still in the future); `stop()` emits an immediate NoteOff for each so voices don't ring forever when transport halts mid-clip. Loop wraps are handled by the caller invoking `reset({startSample, startBeat})`. **The anchor must be the monotonic sample clock (`EngineClient.currentMonotonicSamples()`), not the wrapping playhead — otherwise post-wrap events all collapse to "fire immediately."**
- `AudioClipScheduler.ts` (Phase 7) — Sibling of `ClipScheduler` for audio clips. Scans clips with a `sampleId` and emits paired `EVT_AUDIO_ON/OFF` frames (voiceId + sampleHash + channelHash + startFrame + gain). Mirrors the MIDI scheduler's anchor/horizon/loop-reset. A one-time **straddle scan** on `start()`/`reset()` triggers a clip already underway at the anchor beat from the correct intra-sample `startFrame`, so seeking into a clip (or a loop region starting mid-clip) plays it instead of dropping it.
- `AudioClipPlayer.ts` (Phase 7) — Worklet-side voice player. Holds decoded interleaved PCM keyed by `channelHash(sampleId)` (shipped via `LOAD_SAMPLE`) and a list of active voices; `render(blockSize, mixInto)` reads each voice forward and adds it into the channel input bus (resolved by hash), auto-stopping at sample end. Called from inside `MixerRouter.processBlock` (via the optional `aux` source) right after the generator chains, so audio flows through the channel's FX/pan/fader/sends like a generator. RT-safe: no per-block allocation (dead voices compacted in place).
- `computePeaks.ts` / `generateDemoSample.ts` / `decodeAudioFile.ts` (Phase 7) — Pure waveform peak binning (Int8 min/max pairs); the deterministic procedural demo sample; and file decode → engine-rate interleaved PCM (browser-only).
- `MidiInput.ts` — WebMIDI bridge. Decodes 0x90 / 0x80 / velocity-0-NoteOn into engine NoteOn/Off events routed to a target numericId from a `getTargetNumericId` callback. `attach(input)` wires an input's `onmidimessage`. Pure logic; tests stub the input.
- `diffInstanceParams.ts` — Pure helper that diffs two `instanceId → params[]` snapshots and emits one InstanceParamChange per modified param, skipping unloaded / newly-loaded instances. App.jsx uses it to forward coordinator-side param mutations into the engine via `engine.setParam(instanceId, paramIndex, value)`.
- `routingConfig.ts` — Pure helpers `buildRoutingConfig(tracks, channels)` + `topoSortChannels(channels)` lifted out of App.jsx so the topo sort, send fan-out, and per-destination level threading are unit-testable.
- `channelHash.ts` — FNV-1a 32-bit hash mirrored worklet ↔ main thread so the meter ring stays a flat 16-byte frame keyed by hash, not a variable-length channel id.
- `EngineEvent.ts` (Phase 6a refactor) — Bytes 4–7 of every 32-byte frame now carry `sampleTime` (absolute u32 sample count) on the main → worklet path. The worklet rewrites them to per-block `frameOffset` (via `rewriteFrameOffset`) before queueing into a plugin's per-block buffer, so the plugin ABI is untouched.
- `channelHash.ts` (Phase 6a) — FNV-1a 32-bit hash mirrored worklet ↔ main thread so the meter ring stays a flat 16-byte frame keyed by hash, not a variable-length channel id.
- `PluginRegistry.ts` — Main-thread catalog of installed plugins. `install` / `has` / `get` / `list`, plus `static loadBuiltin(baseUrl)` for the canonical `plugin.json` + `plugin.wasm` + `ui/<entry>` folder layout, and `static loadFromOpfsViaSw(pluginId, version)` (Phase 6d) that fetches the manifest + wasm through the Service Worker's `/_noa/plugins/<id>/<version>/{manifest,wasm}` namespace. The latter is how user-installed `.noaplugin` plugins reach the engine.
- `WorkletProtocol.ts` — Pure protocol class wrapping a `MessagePort`-shaped object. `loadPlugin(args)` posts `INSTANTIATE_PLUGIN` (carrying numericId + chainId + slot) and awaits the matching `INSTANCE_READY`. `updateRouting(config)` posts `UPDATE_ROUTING`. `applyPresetState({chainId, slot, stateBytes})` for ABI v1.1 hot-swap. Used by EngineClient; tested with a hand-rolled port stub.
...
- `projectModel.ts` — `Project` shape (tracks, clips, channels, **samples**, bpm, loop, metronome) + `seedProject()` that materializes the data.js demo as a typed value. `Sample` is lightweight metadata (`{id, name, source: 'synth'|'import', channels, sampleRate, frames, durationSec, freq?}`); the raw PCM lives in the worklet + OPFS, never in the project. `CURRENT_SCHEMA_VERSION` is 4.
...
- `SampleStore.ts` (Phase 7) — Persists imported audio PCM in OPFS at `/samples/<id>/pcm.bin` (12-byte header `[channels, frames, sampleRate]` + interleaved Float32). Pure `encodeSample`/`decodeSample` (the latter validates header vs payload) + a thin I/O wrapper (`write`/`read`/`has`/`remove`/`list`). Opened via `openSampleStore.js`. Main-thread only (decode needs an AudioContext, unavailable in workers).
